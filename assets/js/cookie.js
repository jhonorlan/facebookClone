...
function manageCokkie(){
	$.ajax({
		url:"data/getSession.php",
		type:"GET",
		success:function(session){
		var read = cm_readCookie("users");
		var getCookie = cookiemonster.get("users");
		var users = [];
			if(read == null){
				users.push(session)
				cookiemonster.set('users',users, 30);
			}else{
				if(!getCookie.includes(session)){
					for(var i = 0; i < getCookie.length; i++){
						users.push(getCookie[i]);
						users.push(session);
					}
					cookiemonster.set('users',users, 30);
				}
